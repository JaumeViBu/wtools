...
describe("extract", () => {
	...
	it("should report conflicts if id is duplicated and different descriptions", async () => {
		...
		const { _, conflicts } = await extract([path]);
	...
	it("should return null if paths is not an array of strings", async () => {
		expect(await extract([42])).toBeNull();
		expect(
			await extract(["./test/samples/extract_sample_xs.txt", 42]),
		).toBeNull();
		expect(await extract([null])).toBeNull();
		expect(await extract([undefined])).toBeNull();
		expect(await extract([{}])).toBeNull();
	...
	it("should return null if path is not a valid filepath", async () => {
		expect(
			await extract([
				"./test/samples/extract_sample_xs.txt",
				"./test/samples/notfound.txt",
			]),
		).toBeNull();
		...
		expect(
			await extract([
				"./test/samples/extract_sample_xs.txt",
				join("test", "samples", "notfound.txt"),
			]),
		).toBeNull();
	});
	...
	it("should return null if options is not an object", async () => {
		...
		expect(
			await extract(["./test/samples/extract_sample_xs.txt"], null),
		).toBeNull();
		expect(
			await extract(["./test/samples/extract_sample_xs.txt"], []),
		).toBeNull();
		expect(
			await extract(["./test/samples/extract_sample_xs.txt"], () => {}),
		).toBeNull();
		expect(
			await extract(["./test/samples/extract_sample_xs.txt"], true),
		).toBeNull();
		expect(
			await extract(["./test/samples/extract_sample_xs.txt"], false),
		).toBeNull();
		expect(
			await extract(["./test/samples/extract_sample_xs.txt"], NaN),
		).toBeNull();
	});
});
